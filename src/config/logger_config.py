...
LOGGING = {
    ...
    "root": {"level": "INFO", "handlers": [], "filters": [], "propagate": True},
    "loggers": {
        "uvicorn": {"level": "INFO", "handlers": [], "filters": [], "propagate": False}
    },
    "filters": {
        "debugFilter": {"()": "tools.logger.logger.LogLevelFilter", "logs_level": 10},
        "infoFilter": {"()": "tools.logger.logger.LogLevelFilter", "logs_level": 20},
        "errorFilter": {"()": "tools.logger.logger.LogLevelFilter", "logs_level": 40},
    },
}
...
def get_standard_logging_config() -> dict:
    ...
    logger_config["root"]["handlers"].extend(["console_standard_handler"])
    logger_config["root"]["filters"].extend(["debugFilter"])

    logger_config["loggers"]["uvicorn"]["handlers"].extend(["console_standard_handler"])
