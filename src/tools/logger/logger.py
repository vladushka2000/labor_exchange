...
class CustomJSONFormatter(jsonlogger.JsonFormatter):
    ...
    def add_fields(
        ...
        message = (
            ...
            else f"Ошибка: {extra_fields.get('err_msg')}"
        )
...
class ExtraFormatter(CustomJSONFormatter):
    ...
    def formatException(
        self, ei: tuple[type[BaseException], BaseException, Traceback | None]
    ) -> str:
        """
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        message = f"{datetime.datetime.fromtimestamp(record.created).strftime('%H:%M:%S.%f')} {record.levelname} {s['msg']}"
        ...
        extra = {
            ...
            if key not in self.COMMON_RECORD_ATTRS
        }
